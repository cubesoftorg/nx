...
export default async function offlineExecutor(options: InvokeLocalExecutorSchema, context: ExecutorContext) {
    try {
...
async function invoke(options: InvokeLocalExecutorSchema, context: ExecutorContext) {
    const serverless = new Serverless({
        workspaceRoot: context.root,
        cwd: resolve(context.root, options.outputPath)
    });
    return serverless.run(ServerlessCommand.InvokeLocal, parseArgs(options));
}
